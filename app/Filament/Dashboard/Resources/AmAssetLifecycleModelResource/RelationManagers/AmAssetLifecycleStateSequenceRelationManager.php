...
namespace App\Filament\Dashboard\Resources\AmAssetLifecycleModelResource\RelationManagers;
...
class AmAssetLifecycleStateSequenceRelationManager extends RelationManager
{
    protected static string $relationship = 'sequences';

    protected static ?string $title = 'Asset Lifecycle State Sequences';

    protected static ?string $modelLabel = 'Asset Lifecycle State Sequence';

    protected static ?string $pluralModelLabel = 'Asset Lifecycle State Sequences';

    public function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make('Asset Lifecycle State Selection')
                    ->schema([
                        Forms\Components\Select::make('lifecycle_state_id')
                            ->label('Asset Lifecycle State')
                            ->required()
                            ->options(function (string $operation, ?Model $record = null) {
                                ...
                                if ($operation === 'edit' && $record instanceof AmAssetLifecycleStateSequence) {
                                    $linkedStateIds = array_filter($linkedStateIds, fn($id) => $id !== $record->lifecycle_state_id);
                                }

                                // Return only unlinked lifecycle states
                                return AmAssetLifecycleState::where('tenant_id', $tenant->id)
                                    ->whereNotIn('id', $linkedStateIds)
                                    ->pluck('lifecycle_state', 'id');
                            })
                            ->searchable()
                            ->preload()
                            ->helperText('Only shows asset lifecycle states not yet added to this sequence')
                            ->columnSpan(['md' => 1]),
                        ...
                        Forms\Components\TextInput::make('line')
                            ...
                            ->helperText('Order in which this state appears in the asset lifecycle')
                            ->columnSpan(['md' => 1]),
                    ])
                    ->columns(['md' => 2]),
                    ...
                    ->schema([
                        Forms\Components\Placeholder::make('current_sequences')
                            ...
                            ->content(function () {
                                ...
                                /** @var Tenant $tenant */
                                $tenant = Filament::getTenant();
                                $totalStates = AmAssetLifecycleState::where('tenant_id', $tenant->id)->count();
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('lifecycleState.asset_active')
                    ->label('Asset Active')
                    ->badge()
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('lifecycleState.asset_active')
                    ->label('Asset Active Status')
                    ->options([
                        1 => 'Active',
                        0 => 'Inactive',
                    ]),
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ...
                    ->using(function (array $data, string $model): Model {
                        ...
                        $existing = AmAssetLifecycleStateSequence::where([
                            'lifecycle_model_id' => $this->getOwnerRecord()->id,
                            'lifecycle_state_id' => $data['lifecycle_state_id'],
                        ])->first();

                        if ($existing) {
                            throw new \Exception('This asset lifecycle state is already in the sequence.');
                        }

                        // Check if line number is already taken
                        $existingLine = AmAssetLifecycleStateSequence::where([
                            'lifecycle_model_id' => $this->getOwnerRecord()->id,
                            'line' => $data['line'],
                        ])->first();
                        ...
                        return AmAssetLifecycleStateSequence::create($data);
                    }),
            ])
            ->actions([
                Tables\Actions\EditAction::make()
                    ->mutateFormDataUsing(function (AmAssetLifecycleStateSequence $record, array $data): array {
                        $data['tenant_id'] = Filament::getTenant()->id;
                        return $data;
                    })
                    ->using(function (AmAssetLifecycleStateSequence $record, array $data): Model {
            ...
            ->emptyStateHeading('No Asset Lifecycle States Configured')
            ->emptyStateDescription('Add asset lifecycle states to define the sequence for this model.')
    ...
    public function reorderTable(array $order): void
    {
        DB::transaction(function () use ($order) {
            ...
            /** @var AmAssetLifecycleModel $lifecycleModel */
            ...
            $recordsToUpdate = AmAssetLifecycleStateSequence::whereIn('id', array_keys($updates))
